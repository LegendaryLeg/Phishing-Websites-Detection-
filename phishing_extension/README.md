...
# Table Of Contents 
- Project Overview
- Install and Use (Chrome Extension)
- Dataset
- Technologies Used
- Algorithm
- Results
...
# Install and Use (Chrome Extension)
The detector also runs as a local Chrome extension. To install it, open Chrome and go to `chrome://extensions`, turn on **Developer mode**, click **Load unpacked**, and select the extension folder that contains `manifest.json` (for a clean install pack, use `phishing_extension`; otherwise you can load this project folder). After loading, open any `http://` or `https://` page — the extension scans automatically and shows a phishing likelihood score (Low / Medium / High) in the popup, along with short reasons. Use **Rescan** to refresh the current tab, and **Trust this site** to add the domain to your personal allowlist so familiar sites are less likely to be flagged. All analysis runs on your device; no browsing data is sent to a server.
